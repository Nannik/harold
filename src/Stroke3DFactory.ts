...
export class Stroke3DFactory
{
    ...
    public static createSkyStrokeMesh(stroke2D: Stroke2D, camera: gfx.Camera, skyRadius: number): gfx.Mesh3
    {
		...
		const vert = [];
		const normals = [];
		for (const v of stroke2D.vertices) {
			ray.setPickRay(v, camera);
			const newV = ray.intersectsSphere(sphere);
			if (!newV) throw new Error('')

			vert.push(newV);
			
			const n = gfx.Vector3.multiplyScalar(newV, -1)
			n.normalize();
			normals.push(n);
		}
    ...
    public static createBillboard(stroke2D: Stroke2D, camera: gfx.Camera, anchorPointWorld: gfx.Vector3): Billboard
    {
		...
		const cameraWorldPosition = camera.localToWorldMatrix.transformPoint(gfx.Vector3.ZERO);

		const cameraToAnchor = gfx.Vector3.subtract(cameraWorldPosition, anchorPointWorld)
		cameraToAnchor.normalize();
		const planeNormal = new gfx.Vector3(cameraToAnchor.x, 0, cameraToAnchor.z)
		...
		const plane = new gfx.Plane3(anchorPointWorld, planeNormal)
		...
		for (const v of stroke2D.vertices) {
			...
			const newV = ray.intersectsPlane(plane)
			if (!newV) throw new Error('');
        ...
        // replace this with your solution        
        return new Billboard(anchorPointWorld, planeNormal, mesh);
    }
